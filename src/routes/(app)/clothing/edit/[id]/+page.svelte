  ...
  onMount(() => {
    const unsubscribe = auth.onAuthStateChanged(async (_user) => {
      if (_user) {
        ...
        itemId = id;
        itemRef = doc(db, 'users', user.uid, 'items', itemId);
...
<main class="form-wrapper">
  ...
  <form on:submit|preventDefault={updateItem}>
    ...
    <input class="form-input" placeholder="Brand" bind:value={brand} on:input={updateBrandSuggestions} list="brandSuggestions" />
    ...
    <select class="form-input" bind:value={lowerCategory} required>
      <option value="" disabled>Select Lower Category</option>
  ...
  {#each colors as c}
    <div
      class="color-circle"
      style="background-color: {c.hex}; border: {color === c.name ? '3px solid #007aff' : '1px solid #ccc'}"
  ...
  {/each}
    ...
    <button type="button" class="form-button danger" on:click={deleteItem}>🗑️ Delete Item</button>
  </form>
</main>
