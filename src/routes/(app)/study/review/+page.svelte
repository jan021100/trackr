<script lang="ts">
  ...
  import { withPaediatricsStudyApproach } from '$lib/paediatrics/studyApproach';
  import { user } from '$lib/stores/user';
  import { PAEDIATRICS_SYLLABUS } from '$lib/paediatrics/paediatricsSyllabus';
  import { cardIntervalPreviews, cardsFromTsv, cardsToTsv, createRetentionCard, removeDuplicateCardInputs, retentionCardFingerprint, reviewCard, todayLocal, validateCardImport, type CardRating, type RetentionCard, type RetentionCardStatus } from '$lib/paediatrics/retentionSchema';
  import { deleteRetentionCard, saveRetentionCard, saveRetentionCards, subscribeToRetentionCards } from '$lib/paediatrics/paediatricsRepository';
  ...
  let front = ''; let back = ''; let context = ''; let tags = ''; let topicId = PAEDIATRICS_SYLLABUS[0].id; let gapId = '';
  ...
  const titleFor = (id: string) => PAEDIATRICS_SYLLABUS.find((topic) => topic.id === id)?.title ?? id;
  ...
  $: filtered = cards.filter((card) => (topicFilter === 'all' || card.topicId.endsWith(topicFilter.toLowerCase())) && (statusFilter === 'all' || card.status === statusFilter) && (!search.trim() || `${card.front} ${card.back} ${card.tags.join(' ')} ${card.topicId}`.toLowerCase().includes(search.toLowerCase()))).sort((a,b) => a.dueDate.localeCompare(b.dueDate));
  $: newForAnki = cards.filter((card) => card.status !== 'archived' && !card.ankiExportedAt).sort((a,b) => a.createdAt.localeCompare(b.createdAt));
  ...
  async function exportTsv(reExport = false){
    if(!uid||saving)return;
    const exporting = reExport ? cards.filter((card)=>card.status!=='archived') : newForAnki;
    if(!exporting.length){notice='No new cards are waiting for Anki export.';return;}
    if(reExport&&!confirm(`Re-export all ${exporting.length} non-archived cards? Use this only to replace a lost file or rebuild your Anki deck.`))return;
    const exportedAt=new Date().toISOString(),batchId=crypto.randomUUID();
    saving=true;
    try{
      download(`trackr-paediatrics-anki-${reExport?'all':'new'}-${todayLocal()}.txt`,cardsToTsv(exporting));
      await saveRetentionCards(uid,exporting.map((card)=>({...card,ankiExportedAt:exportedAt,ankiExportBatchId:batchId,updatedAt:exportedAt})));
      notice=`Exported ${exporting.length} ${reExport?'cards':'new card'+(exporting.length===1?'':'s')} to Anki.`;
    }catch(e){fail(e)}finally{saving=false}
  }
  async function markExistingExported(){
    if(!uid||saving||!newForAnki.length)return;
    if(!confirm(`Mark all ${newForAnki.length} currently unexported cards as already present in Anki? No file will be downloaded. Use this once if you imported these cards before Trackr began recording exports.`))return;
    const exportedAt=new Date().toISOString(),batchId=`baseline-${crypto.randomUUID()}`;
    saving=true;
    try{await saveRetentionCards(uid,newForAnki.map((card)=>({...card,ankiExportedAt:exportedAt,ankiExportBatchId:batchId,updatedAt:exportedAt})));notice=`Marked ${newForAnki.length} existing cards as already exported. Future exports will contain new cards only.`;}catch(e){fail(e)}finally{saving=false}
  }
  ...
  async function copyCardPrompt(){const prompt=`You generate retention cards for my Trackr Paediatrics State Exam tracker.
...
ALLOWED TOPIC IDS:
- 1a, 1b, 1c through 40a, 40b, 40c (120 separate topics)
...
- Do not create irrelevant trivia, duplicates, vague prompts, yes/no questions, or cards with several unrelated facts. Basic terms, physical findings and named common examples are valid targets when they repair a real gap.
...
- Generate only cards justified by the study material, mistakes, or weak spots I give you.`;try{await navigator.clipboard.writeText(withPaediatricsStudyApproach(prompt, 'cards'));notice='Card-generation prompt copied for ChatGPT.';}catch{error='Clipboard access was unavailable.'}}
</script>
...
<main class="review-shell">
  ...
  {:else if mode==='create'}
    <div class="create-grid"><section class="panel form"><h2>Create linked retention card</h2><div class="two"><label>Topic<select bind:value={topicId}>{#each PAEDIATRICS_SYLLABUS as t}<option value={t.id}>{t.id} · {t.title}</option>{/each}</select></label><label>Gap ID (optional)<input bind:value={gapId} placeholder="Links this card to a weak spot" /></label></div><label>Front<textarea bind:value={front} placeholder="One focused retrieval prompt"></textarea></label><label>Back<textarea bind:value={back} placeholder="Concise expected answer"></textarea></label><label>Clinical context (optional)<textarea bind:value={context}></textarea></label><label>Tags<input bind:value={tags} placeholder="oral-follow-up differential-diagnosis" /></label><button class="primary" disabled={saving||!front.trim()||!back.trim()} on:click={createCard}>Create card</button></section><section class="panel form"><div class="import-title"><div><p>CHATGPT IMPORT</p><h2>Import generated cards</h2></div><button on:click={copyCardPrompt}>Copy card-generation prompt</button></div><span class="hint">Paste the JSON returned by ChatGPT. Trackr validates every topic ID and metadata field before saving anything.</span><textarea class="json" bind:value={chatGptJson} placeholder={'{"kind":"trackr-retention-cards","schemaVersion":1,"cards":[{"topicId":"1b","front":"…","back":"…","tags":["weakspot"]}]}' }></textarea><button class="primary" disabled={saving||!chatGptJson.trim()} on:click={importChatGptCards}>Validate & import cards</button></section></div>
  {:else}
    {#if newForAnki.length}<div class="export-baseline"><span>Already imported the current cards before export tracking was added?</span><button disabled={saving} on:click={markExistingExported}>Mark current cards as already exported</button></div>{/if}
    <section class="panel"><div class="anki-export"><div><p>ANKI HANDOFF</p><h2>{newForAnki.length} new {newForAnki.length===1?'card':'cards'} ready</h2><span>Trackr remembers each exported card. The main export never includes previously exported cards.</span></div><div class="export-actions"><button class="primary" disabled={saving||!newForAnki.length} on:click={()=>exportTsv(false)}>Export {newForAnki.length} new</button><button disabled={saving||!cards.length} on:click={()=>exportTsv(true)}>Re-export all</button></div></div><div class="tools"><input bind:value={search} placeholder="Search cards and tags"/><select bind:value={topicFilter}><option value="all">All blocks</option><option>A</option><option>B</option><option>C</option></select><select bind:value={statusFilter}><option value="all">All statuses</option><option value="active">Active</option><option value="suspended">Suspended</option><option value="mastered">Mastered</option><option value="archived">Archived</option></select><button on:click={()=>importInput.click()}>Import Anki text</button><input class="hidden" bind:this={importInput} type="file" accept=".txt,.tsv,.csv,text/plain,text/tab-separated-values" on:change={importTsv}/></div><p class="hint">UTF-8 Anki text format with declared tab separator, HTML line breaks and an assigned Tags column. Front and Back remain the first two note fields; Trackr metadata is retained in additional columns. Anki also detects a repeated file by its unchanged Front field.</p><div class="library">{#each filtered as card}<article><div><b>{card.topicId}</b><h3>{card.front}</h3><p>{card.back}</p><small>Due {card.dueDate} · {card.reviews.length} reviews · {card.ankiExportedAt?`Exported to Anki ${new Date(card.ankiExportedAt).toLocaleDateString('en-GB')}`:'Not exported'} · {card.tags.join(' · ')}</small></div><div class="card-actions"><select value={card.status} on:change={(e)=>setStatus(card,(e.currentTarget as HTMLSelectElement).value as RetentionCardStatus)}><option value="active">Active</option><option value="suspended">Suspended</option><option value="mastered">Mastered</option><option value="archived">Archived</option></select><button class="delete" disabled={saving} on:click={()=>removeCard(card)}>Delete</button></div></article>{/each}</div></section>
  {/if}
</main>
...
<style>
  ...
  .anki-export{display:flex;justify-content:space-between;align-items:center;gap:20px;padding:16px;margin-bottom:14px;border:1px solid #303746;border-radius:14px;background:linear-gradient(110deg,rgba(36,42,57,.9),rgba(20,24,33,.9))}.anki-export p{margin:0 0 5px;color:#929aff;font-size:.64rem;font-weight:800;letter-spacing:.14em}.anki-export h2{margin:0 0 4px;font-size:1.15rem}.anki-export span{color:#8991a4;font-size:.74rem}.export-actions{display:flex;gap:7px;flex-shrink:0}.export-actions button{border:1px solid #343a48;background:#212630;color:#eef0f7;border-radius:9px;padding:10px 12px;cursor:pointer}.export-actions .primary{margin:0}.export-actions button:disabled{opacity:.45;cursor:not-allowed}
  .export-baseline{display:flex;justify-content:space-between;align-items:center;gap:14px;margin-bottom:9px;padding:10px 13px;border:1px solid #4b422d;border-radius:11px;background:#282319;color:#c9b98b;font-size:.72rem}.export-baseline button{border:1px solid #655837;border-radius:8px;background:#312a1d;color:#e0cc93;padding:8px 10px;cursor:pointer}.export-baseline button:disabled{opacity:.45}
  ...
  @media(max-width:850px){.create-grid{grid-template-columns:1fr}.import-title{display:block}.import-title button{margin-top:10px}.anki-export{align-items:flex-start}.export-actions{flex-direction:column}}
