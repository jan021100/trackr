...
export const DEFAULT_ANKI_GAP_DECK = 'Trackr::Paediatrics Gaps';
export const DEFAULT_ANKI_GAP_MODEL = 'Trackr Paediatrics Gap';
...
async function discoverNotes(settings: AnkiConnectSettings, cards: RetentionCard[]) {
  ...
  const tagged = await invokeAnki<number[]>(settings, 'findNotes', { query: 'tag:trackr::paediatrics-gap' });
  ...
  // Bind only the requested Paediatrics cards. Never let unrelated subject cards
  // enter progress or time totals, even if a supplied ID or stored Anki ID overlaps.
  for (const card of cards) {
    const safe=ankiTagPart(card.id);
    const matched=notes.find(note=>note.tags.includes('trackr::paediatrics-gap')
      && note.fields.TrackrTopicId?.value?.trim()===card.topicId
      && note.fields.TrackrGapId?.value?.trim()===card.gapId
      && (noteTrackrCardId(note)===card.id || noteTrackrCardId(note)===safe || note.tags.includes(`trackr-id::${safe}`)));
...
/**
 * Reads absolute review-time totals for every Trackr Paediatrics gap card that has
 * ever been linked to Anki. Reconciled subject/topic/gap identity keeps time
 * attributable even when notes move to another deck.
 */
